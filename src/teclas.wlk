...
object teclas {
	...
	method teclasFantasmas() {
		keyboard.f().onPressDo { // spawnear más fantasmas giratorios
			1.times { n =>
				const vel = 10.randomUpTo(50)
			 	const fantasma = new Fantasma(x0=game.center().x()-1 + n*3, y0=game.center().y()+65, vel_x0=vel, hayFriccion=false)
	...
	method teclasJugador() {
		const jugador = registry.get("jugador")
		...
		keyboard.t().onPressDo {
			console.println(jugador+": rapidez = "+jugador.rapidez())
